...
# local destination
if [ ! -d "$HARFBUZZ_HTML5_DST/lib-$UE_EMVER_LIBPATH" ]; then
	mkdir -p "$HARFBUZZ_HTML5_DST/lib-$UE_EMVER_LIBPATH"
fi
...
#	TYPE=${type^^} # OSX-bash doesn't like this
	...
	cp ../libharfbuzz.$UE_LIB_EXT "$HARFBUZZ_HTML5_DST"/lib-$UE_EMVER_LIBPATH/libharfbuzz${SUFFIX}.$UE_LIB_EXT
	cd ..
}

if [ "$USE_INTERMEDIATE_PATH" == "1" ]; then
	mkdir -p $HOME/$UE_EMVER_LIBPATH/$HARFBUZZ_VERSION
	cd $HOME/$UE_EMVER_LIBPATH/$HARFBUZZ_VERSION
fi
...
ls -l "$HARFBUZZ_HTML5_DST/lib-$UE_EMVER_LIBPATH"
