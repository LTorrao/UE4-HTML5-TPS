...
if [ ! -d "$ZLIB_HTML5_DST/lib-$UE_EMVER_LIBPATH" ]; then
	mkdir -p "$ZLIB_HTML5_DST/lib-$UE_EMVER_LIBPATH"
fi
...
#	TYPE=${type^^} # OSX-bash doesn't like this
	...
	cp libz.$UE_LIB_EXT "$ZLIB_HTML5_DST"/lib-$UE_EMVER_LIBPATH/zlib${SUFFIX}.$UE_LIB_EXT
...
if [ "$USE_INTERMEDIATE_PATH" == "1" ]; then
	mkdir -p $HOME/$UE_EMVER_LIBPATH/$ZLIB_TAR_NAME
	cd $HOME/$UE_EMVER_LIBPATH/$ZLIB_TAR_NAME
fi
...
ls -l "$ZLIB_HTML5_DST/lib-$UE_EMVER_LIBPATH"
