...
# local destination
if [ ! -d "$ICU_HTML5_DST/lib-$UE_EMVER_LIBPATH" ]; then
	mkdir -p "$ICU_HTML5_DST/lib-$UE_EMVER_LIBPATH"
fi
...
#	EMFLAGS="$UE_EMFLAGS -fno-except"
	...
	cp ../libicu.$UE_LIB_EXT "$ICU_HTML5_DST"/lib-$UE_EMVER_LIBPATH/libicu${SUFFIX}.$UE_LIB_EXT
	cd ..
}

if [ "$USE_INTERMEDIATE_PATH" == "1" ]; then
	mkdir -p $HOME/$UE_EMVER_LIBPATH/$ICU_VERSION
	cd $HOME/$UE_EMVER_LIBPATH/$ICU_VERSION
fi
...
ls -l "$ICU_HTML5_DST/lib-$UE_EMVER_LIBPATH"
