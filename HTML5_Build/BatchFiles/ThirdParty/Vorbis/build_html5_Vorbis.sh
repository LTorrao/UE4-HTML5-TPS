...
# local destination
if [ ! -d "$VORBIS_HTML5_DST/lib-$UE_EMVER_LIBPATH" ]; then
	mkdir -p "$VORBIS_HTML5_DST/lib-$UE_EMVER_LIBPATH"
fi
...
#	TYPE=${type^^} # OSX-bash doesn't like this
	...
	emcmake cmake -G "$EM_CMAKE_GEN_TYPE" \
		...
		-DOGG_LIBRARIES=$OGG_HTML5_DST/lib-$UE_EMVER_LIBPATH \
	...
	cp lib/libvorbis.$UE_LIB_EXT "$VORBIS_HTML5_DST"/lib-$UE_EMVER_LIBPATH/libvorbis${SUFFIX}.$UE_LIB_EXT
#	cp lib/libvorbisenc.$UE_LIB_EXT "$VORBIS_HTML5_DST"/lib-$UE_EMVER_LIBPATH/libvorbisenc${SUFFIX}.$UE_LIB_EXT
	cp lib/libvorbisfile.$UE_LIB_EXT "$VORBIS_HTML5_DST"/lib-$UE_EMVER_LIBPATH/libvorbisfile${SUFFIX}.$UE_LIB_EXT
	cd ..
}

if [ "$USE_INTERMEDIATE_PATH" == "1" ]; then
	mkdir -p $HOME/$UE_EMVER_LIBPATH/$VORBIS_VERSION
	cd $HOME/$UE_EMVER_LIBPATH/$VORBIS_VERSION
fi
...
ls -l "$VORBIS_HTML5_DST/lib-$UE_EMVER_LIBPATH"
