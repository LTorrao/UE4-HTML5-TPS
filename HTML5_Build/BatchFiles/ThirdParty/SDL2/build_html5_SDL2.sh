...
# local destination
if [ ! -d "$SDL2_HTML5_DST/lib-$UE_EMVER_LIBPATH" ]; then
	mkdir -p "$SDL2_HTML5_DST/lib-$UE_EMVER_LIBPATH"
fi
...
#	TYPE=${type^^} # OSX-bash doesn't like this
	...
	cp libSDL2${DSUFFIX}.$UE_LIB_EXT "$SDL2_HTML5_DST"/lib-$UE_EMVER_LIBPATH/libSDL2${SUFFIX}.$UE_LIB_EXT
	cd ..
}

if [ "$USE_INTERMEDIATE_PATH" == "1" ]; then
	mkdir -p $HOME/$UE_EMVER_LIBPATH/$SDL2_VERSION
	cd $HOME/$UE_EMVER_LIBPATH/$SDL2_VERSION
fi
...
ls -l "$SDL2_HTML5_DST/lib-$UE_EMVER_LIBPATH"
